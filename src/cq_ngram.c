...
static void ht_record(freq_ht_t *ht, const char *text, size_t len)
{
    ...
    for (uint32_t probe = 0; probe < HT_SLOTS; probe++) {
        ...
        if (s->pool_off == SLOT_EMPTY) {
            ...
            s->str_len = (uint32_t)len; s->frequency = 1;
...
/* JSON — extract every '"…"'-delimited span (keys and string values). */
static void extract_json(const char *input, size_t input_len, freq_ht_t *ht)
{
    size_t i = 0;
    while (i < input_len) {
 ...
 */
static void extract_csv(const char *input, size_t input_len, freq_ht_t *ht)
{
    size_t i = 0;
    while (i < input_len) {
        ...
        if (input[i] == '"') {
            ...
            if (len > 0) ht_record(ht, input + start, len);
            while (i < input_len && (input[i] == ',' || input[i] == '\r')) i++;
            if (i < input_len && input[i] == '\n') i++;
        }
    }
 ...
 */
static void extract_log(const char *input, size_t input_len, freq_ht_t *ht)
{
    size_t i = 0;
    while (i < input_len) {
        ...
        size_t len = i - start;
        if (len >= 4) ht_record(ht, input + start, len);   /* skip short tokens */
    }
}
 ...
 */
static void extract_code(const char *input, size_t input_len, freq_ht_t *ht)
{
    size_t i = 0;
    while (i < input_len) {
        char ch = input[i];
        ...
        if (ch == '"' || ch == '\'') {
            ...
            if (j < input_len && j > start) ht_record(ht, input + start, j - start);
            i = j + 1;
            continue;
        }
        ...
        if (isalpha((unsigned char)ch) || ch == '_') {
            ...
            size_t len = i - start;
            if (len >= 4) ht_record(ht, input + start, len);   /* skip short names */
            continue;
        }
 ...
 * Dispatch table
 * -------------------------------------------------------------------------- */
typedef void (*extractor_fn)(const char *, size_t, freq_ht_t *);
                  ...
                  cq_candidate_list_t *out)
{
    ...
    extractors[fmt](input, input_len, ht);
